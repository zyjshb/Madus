...
object AppChangelog {
    ...
    val entries: List<Entry> = listOf(
        Entry(
            ...
            date = "2026-09-23",
            fixed = listOf(
                "未识别曲风也能保存歌曲反馈，按实际选择的版本建立歌曲参照，不再要求先选类别",
                "搜索后认真收听的具体歌曲参与推荐，补齐标签时保留已听时长",
                "开局优先选择近期未反复播放的喜欢歌曲，首曲不再用于探索",
                "限制同一歌曲来源连续推荐；自动学到欧美或日语不再等于喜欢整个语种",
                "使用已补齐的标签，识别更细的音乐类型，降低未主动偏好的加速、变调与翻唱版本排序",
            ),
            known = listOf("旧版提示未识别类型时没有保存反馈，这部分需要重新点选；已有点赞、收藏及听歌记录保留"),
        ),
        Entry(
            version = "1.20.1",
            date = "2026-09-22",
        ...
        Entry(
            ...
            date = "2026-09-22",
            changed = listOf(
                "结合每首歌的平均实际收听、播放比例、有效前台停留、点赞和收藏学习，不再仅取最强一次反馈",
                "搜索类型作为短期弱信号，主动点播后认真收听进一步影响推荐",
                "有合格候选时限制同类连续占满队列，给相近类型保留实际发现机会",
                "修复省流召回、缓存和单首切歌让推荐困在同一类型的问题",
            ),
            known = listOf("旧版本未记录精确收听时长，新统计从升级后的实际播放开始积累"),
        ),
        Entry(
            version = "1.19.1",
            date = "2026-09-22",
