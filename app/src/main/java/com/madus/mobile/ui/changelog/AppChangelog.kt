...
object AppChangelog {
    ...
    val entries: List<Entry> = listOf(
        Entry(
            ...
            date = "2026-09-22",
            added = listOf(
                "推荐页新增「我的音乐口味」，可选曲风、语言和氛围，也可留空随听学习",
                "保存已发现的音乐与收听偏好，重启后继续沿着喜欢的方向找新歌",
                "推荐播放时可直接点「不合口味」，不用展开更多菜单",
            ),
            changed = listOf(
                "点开一首不再算喜欢，实际收听、喜欢和收藏共同影响后续推荐",
                "优先从已有候选调整待播歌曲，后台继续找歌；探索集中在相近曲风",
                "跳过先冷却这首歌，同类只逐步少推，不会因为几首不喜欢就封掉整个曲风或 UP 主",
            ),
            fixed = listOf(
                "待播歌曲不再自动成为兴趣依据，避免越推越偏",
                "小屏推荐页的封面信息可以滚动，播放控件保持固定",
                "切到歌单或搜索后不再显示推荐电台的调整提示",
            ),
